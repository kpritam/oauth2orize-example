...
// id_token grant type.
server.grant(
  oauth2orize_ext.grant.idToken(function(client, user, ares, done) {
    var id_token;
    // Do your lookup/token generation.
    // ... id_token =

    id_token = utils.getUid(256);

    console.log("aki  con  el open  idToken:" + id_token);

    db.accessTokens.save(id_token, user.id, client.clientId, error => {
      if (error) return done(error);
